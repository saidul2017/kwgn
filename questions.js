/**
 * Bank Soal UAS Pendidikan Kewarganegaraan
 * Format:
 * - type: 'mc' (multiple choice) | 'essay'
 * - mc: { question, options: [a,b,c,d,e], correct: index, points, explanation }
 * - essay: { question, points, rubric, keywords[] (untuk fallback grading) }
 */
...
const QUESTIONS = [
    ...
    // ==================== ESSAY (4 soal x 10 = 40 poin) ====================
    {
        ...
        keywords: ['dasar negara', 'pandangan hidup', 'sumber hukum', 'pedoman', 'sila', 'persatuan', 'keadilan', 'musyawarah', 'gotong royong', 'toleransi']
    },
    {
        type: 'essay',
        ...
        keywords: ['musyawarah', 'mufakat', 'BEM', 'HMJ', 'pemilihan', 'organisasi', 'kebebasan', 'pendapat', 'toleransi', 'sila keempat', 'persatuan']
    },
    {
        type: 'essay',
        ...
        keywords: ['integrasi nasional', 'persatuan', 'kesatuan', 'hoaks', 'ujaran kebencian', 'media sosial', 'literasi digital', 'bhinneka tunggal ika', 'toleransi', 'bela negara', 'generasi muda']
    },
    {
        type: 'essay',
        ...
        keywords: ['bela negara', 'cinta tanah air', 'kesadaran berbangsa', 'pancasila', 'rela berkorban', 'belajar', 'produk dalam negeri', 'hoaks', 'disiplin', 'taat hukum', 'pajak', 'lingkungan']
    }
];

// Hitung total poin maksimal
const TOTAL_POINTS = QUESTIONS.reduce((sum, q) => sum + q.points, 0);
const MC_QUESTIONS = QUESTIONS.filter(q => q.type === 'mc');
const ESSAY_QUESTIONS = QUESTIONS.filter(q => q.type === 'essay');
const MC_TOTAL_POINTS = MC_QUESTIONS.reduce((sum, q) => sum + q.points, 0);
const ESSAY_TOTAL_POINTS = ESSAY_QUESTIONS.reduce((sum, q) => sum + q.points, 0);
